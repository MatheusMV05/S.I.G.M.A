...
export const useDailyRevenue = (days: number = 30) => {
  return useQuery({
    ...
    queryFn: () => apiRequest(`/reports/daily-revenue?days=${days}`),
...
export const useTopProducts = (limit: number = 10) => {
  return useQuery({
    ...
    queryFn: () => apiRequest(`/reports/top-products?limit=${limit}`),
    staleTime: 5 * 60 * 1000, // 5 minutos
  });
};
...
export const useHourlySales = () => {
  return useQuery({
    ...
    queryFn: () => apiRequest(`/reports/hourly-sales`),
    staleTime: 2 * 60 * 1000, // 2 minutos
    refetchInterval: 5 * 60 * 1000, // Atualiza a cada 5 minutos
  });
};
...
export const useSalesByCategory = () => {
  return useQuery({
    ...
    queryFn: () => apiRequest(`/reports/sales-by-category`),
    staleTime: 5 * 60 * 1000,
  });
};
...
export const useSalesStatistics = (days: number = 30) => {
  return useQuery({
    ...
    queryFn: () => apiRequest(`/reports/sales-statistics?days=${days}`),
    staleTime: 5 * 60 * 1000,
  });
};
...
export const useSalesDistribution = (days: number = 30) => {
  return useQuery({
    ...
    queryFn: () => apiRequest(`/reports/sales-distribution?days=${days}`),
    staleTime: 5 * 60 * 1000,
  });
};
...
export const useSalesByWeekday = (days: number = 30) => {
  return useQuery({
    ...
    queryFn: () => apiRequest(`/reports/sales-by-weekday?days=${days}`),
    staleTime: 5 * 60 * 1000,
  });
};
...
// ================================================================
// Novos Hooks para Insights Avançados
// ================================================================
...
export function useSazonalidadeMensal(dias: number = 90) {
  return useQuery({
    queryKey: reportKeys.sazonalidadeMensal(dias),
...
export function useSazonalidadeSemanal(dias: number = 60) {
  return useQuery({
    queryKey: reportKeys.sazonalidadeSemanal(dias),
...
export function useSazonalidadeHoraria(dias: number = 30) {
  return useQuery({
    queryKey: reportKeys.sazonalidadeHoraria(dias),
...
export function useProdutosBaixaRotatividade(limit: number = 20) {
  return useQuery({
    queryKey: reportKeys.produtosBaixaRotatividade(limit),
...
export function useAnaliseABC(dias: number = 90) {
  return useQuery({
    queryKey: reportKeys.analiseABC(dias),
